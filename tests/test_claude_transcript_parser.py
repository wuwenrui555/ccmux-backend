"""Tests for ccmux.claude_transcript_parser — pure logic, no I/O."""

import pytest

from ccmux.claude_transcript_parser import (
    ParsedMessage,
    TranscriptParser,
)
...
class TestParseMessage:
    ...
    def test_local_command_invoke(self):
        ...
        assert result.text == ""
        assert result.command_name == "/clear"
...
class TestParseEntries:
    def test_assistant_text(self, make_jsonl_entry, make_text_block):
        ...
        result, pending, _trailing = TranscriptParser.parse_entries(
    ...
    def test_local_command_name_carries_across_calls(
        self, make_jsonl_entry, make_text_block
    ):
        """Regression for B2: in real JSONL, <command-name> and
        <local-command-stdout> live in separate user entries. The monitor
        polls in cycles, so invoke and stdout can land in different
        parse_entries calls; the command name must be carried via
        last_cmd_name or the stdout renders with no command label.
        """
        invoke = make_jsonl_entry(
            "user", [make_text_block("<command-name>/status</command-name>")]
        )
        stdout = make_jsonl_entry(
            "user",
            [make_text_block("<local-command-stdout>all good</local-command-stdout>")],
        )

        # Poll 1: only the invoke arrives. Nothing emitted yet; trailing
        # command name is carried.
        result1, pending1, trailing1 = TranscriptParser.parse_entries(
            [invoke], session_id="sess", pending_tools={}
        )
        assert result1 == []
        assert trailing1 == "/status"

        # Poll 2: stdout arrives. With the carried name, the rendered
        # local_command entry includes "/status".
        result2, _pending2, trailing2 = TranscriptParser.parse_entries(
            [stdout], session_id="sess", pending_tools=pending1, last_cmd_name=trailing1
        )
        assert len(result2) == 1
        assert result2[0].content_type == "local_command"
        assert "/status" in result2[0].text
        assert "all good" in result2[0].text
        assert trailing2 is None
    ...
    def test_pending_tools_flushed_without_carry_over(
        ...
        result, pending, _trailing = TranscriptParser.parse_entries(
            entries, session_id="test-session", pending_tools=None
        )
